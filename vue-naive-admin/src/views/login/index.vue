...
onBeforeMount(() => {
  axios.post('/zhang/login?username=communityAdmin2&password=6666').then
  (
    res=>console.log(res),
    err=>console.log(err)
  )
...
async function handleLogin() {
  ...
  try {
    ...
    console.log(res)
    const ares = await axios.post('/zhang/login?username=communityAdmin2&password=6666')
    console.log(ares.headers.authorization)
    localStorage.setItem('authorization', ares.headers.authorization)
    setToken(ares.headers.authorization)
    // console.log(zr)
    // return 
    if (res.code === 0) {
